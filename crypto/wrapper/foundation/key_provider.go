...
func (obj *KeyProvider) ExportPublicKey(publicKey PublicKey) ([]byte, error) {
	outBuf, outBufErr := newBuffer(int(obj.ExportedPublicKeyLen(publicKey.(PublicKey) /* lg0 */) /* lg2 */))
	if outBufErr != nil {
...
func (obj *KeyProvider) ExportPrivateKey(privateKey PrivateKey) ([]byte, error) {
	outBuf, outBufErr := newBuffer(int(obj.ExportedPrivateKeyLen(privateKey.(PrivateKey) /* lg0 */) /* lg2 */))
	if outBufErr != nil {
		return nil, outBufErr
	}
	defer outBuf.delete()
